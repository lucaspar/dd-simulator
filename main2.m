...
%
datasetName = 'turbid';
basePath = '../'; % must to be set where the dataset will be placed
...
saveLevelDB = true; 
loadFromMat = false; % must to be set as true (when loading from a .mat file) 
                    % or false (when load from image files)
%
%% source config
%
turbidityDatabasePath = 'TurbidityAzul';       % tubidity reference dataset
imageDatabasePathRGB='../RGB';  % base image dataset (can be image files or .mat)
imageDatabasePathDepth='../ConvertedDepth';  % base image dataset (can be image files or .mat)
%
%% Main
%
...
if loadFromMat
    ...
    %baseImages = loadImages(imageDatabasePathRGB,'png');       % load base images from image database folder; Set the file extension
    %distances= loadImages(imageDatabasePathDepth,'png');       % load base images from image database folder; Set the file extension
end
%W = waitbar(0,'Go drink a coffe, it will take a while...'); % Wait bar
%steps=numSample*length(baseImages)*length(turbidityImages);
%
for k = 1:numSample
    imgNames = dir(strcat(imageDatabasePathRGB,'/*.','png'));
    nImages=length(imgNames);
    for i = 1: nImages
        input = double(loadImage(imageDatabasePathRGB,'png', i)) / 255.0;       % load base images from image database folder; Set the file extension
        
        
        distance = double(loadImage(imageDatabasePathDepth,'png', i)) / 255.0;
        distance = distance(:,:,1);
